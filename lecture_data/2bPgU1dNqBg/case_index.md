# Case Index — Lecture 2bPgU1dNqBg (3.371 Spring 2016, session 2/12)

*Citation format: layer 3 paragraph anchor (e.g. `§4.p3`). The lecture is built around the central question "why no 3D printing of metals?" and surfaces a number of cases as teaching examples.*
...
- **Heat intensity ladder** (§8.p4): 0.3 W/cm² (sun) / 100 W/cm² (plumber's torch) / 10⁴ W/cm² (arc weld) / 10⁶ W/cm² (laser, electron beam). This is a recurring framing chart in Eagar's teaching.
- **Volume change on solidification** (§6.p3): 6% (aluminum), 3.5% (iron). Recurring framing pair.
- **Surface energy of metals** (§7.p3): ~1 J/m² — within a small range whether tin-lead solder (0.7) or steel (0.5).
- **Bond/Eötvös crossover length** (§7.p3): ~1/8 inch (3 mm) for typical metals.
- **Langmuir** (§7.p1, §7.p2): ~10 nanoseconds for monolayer adsorption at 1 atm.
- **500 lb/hr** (§10.p3): Tom's calculated deposition rate for multi-megavolt electron-beam 3D printing.
- **400 ksi shockwave** (§10.p4): Grüneisen-effect stress relief.

## Open questions for the case index methodology

- The 2015 vs. 2005 Amtrak Philadelphia derailment dating needs reconciliation with the aggregate's canonical cluster.
- "Boulder, Massachusetts" — likely a captioner error or Tom's slip; the location is probably Boylston or Bolton, MA (no Boulder in MA). Editor should verify.