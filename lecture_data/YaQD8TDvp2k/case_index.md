# Case Index — Lecture 24

*Citation format: layer 3 paragraph anchor (e.g. `§4.p9`). Click-through in the static site reveals the corresponding layer 2 passage with timestamp range.*

This lecture is structured around aluminum welding design and defects, with two extended forensic cases (the cement truck and the tree stand turnbuckle) anchoring the design and porosity discussions respectively.
...
- **Note:** Tom handles this as a single continuous case in this lecture, weaving the Iowa and Pennsylvania incidents together. The case index treats it as one unit accordingly. The canonical cluster list contains multiple proximate entries; cross-corpus reconciliation should determine whether these are facets of one cluster or distinct incidents Tom recounts in different combinations across lectures.
...
- **Anchor:** `§3.p3`
- **Frame:** Two dollars per pound in automobiles; $200 per pound in aircraft; twenty thousand dollars per pound in spacecraft. Recurring framing figure across the corpus, here used to justify why aluminum structures can afford more expensive joint preparation than steel.
- **Note:** This is a framing figure Tom returns to across lectures, not a case.

### Aluminum vs. steel material cost ratio
- **Anchor:** `§3.p3`
- **Frame:** "Aluminum is about five times the expense of steel" by equivalent volume — used to ground the economic argument for more elaborate aluminum weld preparation.

### Hydrogen solubility drop on solidification (aluminum)
- **Anchor:** `§6.p1`
- **Frame:** Factor of 10–15 drop in hydrogen solubility from liquid to solid aluminum, on a semi-log plot, generating porosity. Standard teaching figure.

### Aluminum porosity–strength proportionality
- **Anchor:** `§6.p3`, `§7.p2`
- **Frame:** Strength loss in aluminum welds is directly proportional to area fraction of porosity; aluminum does not suffer hydrogen embrittlement (contrast with steel). Used as the technical foundation for the turnbuckle case.

## Open questions for the case index methodology

- The cement truck cluster appears multiple times in the canonical list under different names. Cross-corpus reconciliation should determine the canonical entry and collapse overlapping ones.
- The Connecticut steering case appears as a discrete cluster in the canonical list (`Connecticut cement truck steering failure`) and is handled here as an embedded sub-story rather than promoted to a top-level case in this lecture. Treatment as a separate cluster matches the canonical list.