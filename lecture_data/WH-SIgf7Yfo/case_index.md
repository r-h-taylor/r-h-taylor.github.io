# Case Index — Lecture WH-SIgf7Yfo (Codes and Standards, Fall 2012, session 6/14)
...
This lecture is built around one extended forensic case (the cement truck pressure vessel, §6–§10) that exemplifies the framework laid out in §1–§5. Several secondary cases are referenced in passing.
...
- **Note:** The cluster name in the aggregate distinguishes this from `Cement truck pressure vessel explosion (Pennsylvania)`, `Initial cement truck tank explosion and worker leg amputation`, and `Connecticut cement truck steering failure`. The earlier amputation in §10.p1 may correspond to `Initial cement truck tank explosion and worker leg amputation` — if so, this lecture treats both as facets of one case. Flag for case-aggregate reconciliation: confirm whether these are one cluster or two.

## Secondary cases referenced in passing
...
- ASME boiler and pressure vessel code 2010 edition: $14,500 plus $600 for binders; estimated 50,000 copies per three-year cycle (§3.p1).
- Section 8 divisions: $680 / $680 / $600 (§3.p2).
- AMS standard: $59 per line of catalog index (§3.p5).
- Stress concentration factor for a hole in an infinite plate: 3, first calculated circa 1870 (§7.p1).
- ASME code pressure vessel safety factor: 2½ to 3½ depending on use (§10.p1).
- Cement truck water tank: 200 gallon, 55 PSI design, aluminum; rupture pressure reached approximately 100–125 PSI (§6.p4, §8.p3).
- Trucks sold across US/Canada/Mexico: 100,000 (§9.p2).

## Open questions for case-aggregate reconciliation

- Is the §10.p1 employee amputation a distinct cluster from the §8.p3 Pennsylvania fatality, or both facets of one cluster? Two candidate cluster names exist in the canon (`Pennsylvania cement truck fatality and regulatory failure` and `Initial cement truck tank explosion and worker leg amputation`); this lecture handles them as connected facets of one company's failure pattern.
- The "bearing manufacturer" back-reference in §4.p7 needs cross-corpus resolution to identify which prior lecture introduced this case.
- The polymer-since-1972 case maps ambiguously to two candidate clusters; reconciliation needed.